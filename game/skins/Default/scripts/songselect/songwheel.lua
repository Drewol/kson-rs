  ...
  if difficulty.scores[1] ~= nil then
    ...
    for i, v in ipairs(grades) do
      if v.max > highScore.score then
        ...
        centreoffset = (oldheight - newheight) / 2 +
            3                                                                                     -- +3 is stupid but ehhh
    ...
    if difficulty.topBadge ~= 0 then
      gfx.BeginPath()
      gfx.ImageRect(x + xOffset + w / 2 - h / 2, y + h / 2 + 5, (h / 2 - 10), h / 2 - 10, badges[difficulty.topBadge], 1,
        0)
    end
...
draw_song = function(song, x, y, w, h, selected)
  ...
  gfx.Translate(x, y)
  gfx.BeginPath()
  gfx.RoundedRectVarying(0, 0, w, h, 0, 0, 0, 40)
  ...
  gfx.DrawLabel(songCache[song.id]["title"], 10, 5, w - 10)
  gfx.DrawLabel(songCache[song.id]["artist"], 20, 50, w - 10)
  gfx.Restore()
  gfx.ForceRender()
end
...
draw_selected = function(song, x, y, w, h)
  ...
  if aspectRatio == "PortraitWidescreen" then
    ...
    --the jacketImage
    draw_diffs(song.difficulties, xpos + xPadding, (ypos + yPadding + imageSize), imageSize,
      math.floor((height / 3) * 1) - yPadding)
  else
    -- difficulty should take up 1/6 of height, full width, and be centered
    draw_diffs(song.difficulties, (w / 2) - (imageSize / 2), (ypos + yPadding + imageSize), imageSize,
      math.floor(height / 6))
  end
  ...
  if aspectRatio == "PortraitWidescreen" then
    gfx.FontSize(40)
    gfx.TextAlign(gfx.TEXT_ALIGN_TOP + gfx.TEXT_ALIGN_LEFT)
    gfx.DrawLabel(songCache[song.id]["title"], xpos + xPadding + imageSize, y + yMargin + yPadding, width - imageSize -
      20)
    gfx.FontSize(30)
    gfx.DrawLabel(songCache[song.id]["artist"], xpos + xPadding + imageSize + 3, y + yMargin + yPadding + 45,
      width - imageSize - 20)
    gfx.FontSize(20)
    gfx.DrawLabel(songCache[song.id]["bpm"], xpos + xPadding + imageSize + 3, y + yMargin + yPadding + 85,
      width - imageSize - 20)
    gfx.FastText(string.format("Effector: %s", diff.effector), xpos + xPadding + imageSize + 3, y + yMargin + yPadding +
      115)
  else
    gfx.FontSize(40)
    gfx.TextAlign(gfx.TEXT_ALIGN_TOP + gfx.TEXT_ALIGN_LEFT)
